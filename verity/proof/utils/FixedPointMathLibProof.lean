...
namespace proof.utils.FixedPointMathLibProof
...
open src.utils
open spec.utils.FixedPointMathLibSpec
open src.utils.FixedPointMathLib

attribute [local simp] maxUint256 saturatingAdd saturatingMul saturatingSub
  src.utils.FixedPointMathLib.dist sqrt clamp
attribute [local simp] src.utils.FixedPointMathLibBase.maxUint256
  src.utils.FixedPointMathLibBase.saturatingAdd
  src.utils.FixedPointMathLibBase.saturatingMul
  src.utils.FixedPointMathLibBase.saturatingSub
  src.utils.FixedPointMathLibBase.dist
  src.utils.FixedPointMathLibBase.avg
  src.utils.FixedPointMathLibBase.sqrt
  src.utils.FixedPointMathLibBase.clamp
...
private theorem maxUint256_val :
  ...
  simp [maxUint256, src.utils.FixedPointMathLibBase.maxUint256,
...
private theorem div_maxUint256_val (x : Uint256) (hx : x.val ≠ 0) :
  ...
  have hRaw : (sub 0 1 : Uint256).val = Verity.Stdlib.Math.MAX_UINT256 := by
    simpa [maxUint256, src.utils.FixedPointMathLibBase.maxUint256] using maxUint256_val
  rw [hRaw]
...
theorem saturatingAdd_saturates_at_uint256_max (x y : Uint256) (s : ContractState) :
  ...
  · have hBranch : y.val > (sub maxUint256 x).val := by
    ...
    have hBranchRaw : (sub (sub 0 1) x).val < y.val := by
      simpa [maxUint256, src.utils.FixedPointMathLibBase.maxUint256] using hBranch
  ...
  · have hNoOverflow : x.val + y.val ≤ Verity.Stdlib.Math.MAX_UINT256 := by
    ...
    have hNotBranchRaw : ¬ (sub (sub 0 1) x).val < y.val := by
      simpa [maxUint256, src.utils.FixedPointMathLibBase.maxUint256] using hNotBranch
    have hAddLt :
...
theorem dist_is_absolute_difference (x y : Uint256) (s : ContractState) :
    dist_property x y ((src.utils.FixedPointMathLib.dist x y).run s).fst := by
  ...
  · have hBranch : x >= y := hGe
    ...
    refine ⟨?_, ?_⟩
    · intro h
      simp [src.utils.FixedPointMathLib.dist, Contract.run, Verity.bind, Bind.bind, Verity.pure,
        Pure.pure,
        hBranch, hSubVal]
      omega
    · intro _h
...
private theorem sqrt_run_eq_sourceUint (x : Uint256) (s : ContractState) :
  ...
  rw [sqrt, src.utils.FixedPointMathLibBase.sqrt.eq_1]
...
private theorem cbrt_run_eq_sourceUint (x : Uint256) (s : ContractState) :
  ...
  rw [cbrt, src.utils.FixedPointMathLibBase.cbrt.eq_1]
...
private theorem log2_eq_searchContract (x : Uint256) :
    src.utils.FixedPointMathLibBase.log2 x = log2SearchContract 7 0 x := by
  simp [src.utils.FixedPointMathLibBase.log2, log2SearchContract, uintOfNat,
...
private theorem log2Up_eq_searchContract (x : Uint256) :
    src.utils.FixedPointMathLibBase.log2Up x = log2UpSearchContract 7 x 0 x := by
  simp [src.utils.FixedPointMathLibBase.log2Up, log2UpSearchContract, uintOfNat,
    Bind.bind, Pure.pure, add, Verity.Core.Uint256.add,
    Verity.Core.Uint256.ofNat, OfNat.ofNat, Verity.Core.Uint256.modulus,
    Verity.Core.UINT256_MODULUS]
...
private theorem log10_eq_searchContract (x : Uint256) :
    src.utils.FixedPointMathLibBase.log10 x = log10SearchContract4 0 x := by
  simp [src.utils.FixedPointMathLibBase.log10, log10SearchContract4,
...
private theorem log10Up_eq_log10_roundInline (x : Uint256) :
    src.utils.FixedPointMathLibBase.log10Up x =
      (do
        let r ← src.utils.FixedPointMathLibBase.log10 x
        log10UpRoundInlineContract x r) := by
  unfold src.utils.FixedPointMathLibBase.log10Up log10UpRoundInlineContract
  rfl
...
private theorem log10Up_run_eq_search (x : Uint256) (s : ContractState) :
  ...
  have hLogSuccessRaw :
      (src.utils.FixedPointMathLibBase.log10 x).run s =
        ContractResult.success ((src.utils.FixedPointMathLibBase.log10 x).run s).fst s := by
    rw [log10_eq_searchContract x]
    exact hLogSuccess
  have hLogSuccessRaw' :
      src.utils.FixedPointMathLibBase.log10 x s =
        ContractResult.success ((src.utils.FixedPointMathLibBase.log10 x).run s).fst s :=
    Contract.eq_of_run_success hLogSuccessRaw
  change (Contract.run
      (Verity.bind (src.utils.FixedPointMathLibBase.log10 x)
  ...
  have hRound := log10UpRoundInlineContract_val x
    ((src.utils.FixedPointMathLibBase.log10 x).run s).fst s
...
private theorem log256Base_run_success_fst (x : Uint256) (s : ContractState) :
    (src.utils.FixedPointMathLibBase.log256 x).run s =
      ContractResult.success
        ((src.utils.FixedPointMathLibBase.log256 x).run s).fst s := by
  simp [src.utils.FixedPointMathLibBase.log256, Contract.run,
...
private theorem log256_input_lt_next_power (x : Uint256) (s : ContractState) :
  ...
  have hxLt : x.val < 2 ^ 256 := by
    simpa [Verity.Core.Uint256.modulus, Verity.Core.UINT256_MODULUS] using x.isLt
  simp [log256, src.utils.FixedPointMathLibBase.log256, Contract.run,
    Bind.bind, Pure.pure, shr_val, add, Verity.Core.Uint256.add,
    Verity.Core.Uint256.ofNat, OfNat.ofNat, Verity.Core.Uint256.modulus,
    Verity.Core.UINT256_MODULUS]
...
private theorem log256Up_input_le_power (x : Uint256) (s : ContractState) :
  ...
  have hxLt : x.val < 2 ^ 256 := by
    simpa [Verity.Core.Uint256.modulus, Verity.Core.UINT256_MODULUS] using x.isLt
  simp [log256Up, src.utils.FixedPointMathLibBase.log256Up, Contract.run,
...
theorem log256Up_returns_math_ceil (x : Uint256) (s : ContractState) :
  ...
  · intro hZero
    have hx : x = 0 := Verity.Core.Uint256.ext (by simpa using hZero)
    subst x
    simp [log256Up, src.utils.FixedPointMathLibBase.log256Up,
      src.utils.FixedPointMathLibBase.log256, Contract.run, Bind.bind,
...
theorem fixedPointMathLib_log2_zero_returns_zero_holds (x : Uint256) (s : ContractState) :
  ...
  simp [fixedPointMathLib_log2_zero_returns_zero, log2, Contract.run,
    src.utils.FixedPointMathLibBase.log2, src.utils.FixedPointMathLibBase.log256,
...
theorem fixedPointMathLib_log2Up_zero_returns_zero_holds (x : Uint256) (s : ContractState) :
  ...
  simp [fixedPointMathLib_log2Up_zero_returns_zero, log2Up, Contract.run,
    src.utils.FixedPointMathLibBase.log2Up, src.utils.FixedPointMathLibBase.log2,
    src.utils.FixedPointMathLibBase.log256, Verity.bind, Bind.bind, Verity.pure,
...
theorem fixedPointMathLib_log10_zero_returns_zero_holds (x : Uint256) (s : ContractState) :
  ...
  simp [fixedPointMathLib_log10_zero_returns_zero, log10, Contract.run,
    src.utils.FixedPointMathLibBase.log10, Verity.bind, Bind.bind, Verity.pure,
    Pure.pure]
...
theorem fixedPointMathLib_log10Up_zero_returns_zero_holds (x : Uint256) (s : ContractState) :
  ...
  simp [fixedPointMathLib_log10Up_zero_returns_zero, log10Up, Contract.run,
    src.utils.FixedPointMathLibBase.log10Up, src.utils.FixedPointMathLibBase.log10,
...
theorem fixedPointMathLib_log256Up_zero_returns_zero_holds (x : Uint256) (s : ContractState) :
  ...
  simp [fixedPointMathLib_log256Up_zero_returns_zero, log256Up, Contract.run,
    src.utils.FixedPointMathLibBase.log256Up, src.utils.FixedPointMathLibBase.log256,
    Verity.bind, Bind.bind, Verity.pure, Pure.pure]
...
end proof.utils.FixedPointMathLibProof
